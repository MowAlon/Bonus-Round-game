        ...
        if (destroyIfCollided(bullets[0], bullets[1])) {
          ...
          bullets.forEach(function(bullet) {
            if (bullet && offScreen(bullet)) {
              ...
              if (bullet.lane.alive === true) {
                otherPlayer(bullet.player).reduceHealth(2)
                bullet.lane.alive = false
...
Game.prototype = {
  ...
  reset: function(board, startingHealth, playAgain) {
    ...
    this.board.lanes.forEach(function(lane) {
      lane.alive = true
    })
...
function drawCharacters(context, board) {
  ...
  characters.forEach(function(graphic, index) {
    if (board.lanes[index].alive) {
      drawAnimatedImage(graphic, board.lanes[index].x, board.size.height - graphic.height, context)
    }
  })
}
