...
var Game = function(assets, sounds) {
  ...
  this.paused = false
  this.bombDropTime = 0
  this.bombDelayBase = 300 //milliseconds
  this.bombDelayVariance = .7
  this.bombDelay = this.bombDelayBase
  var playAgain = false
...
Game.prototype = {
  ...
  dropBombsTimed: function(board, assets, sounds) {
    var npc = board.players.find(function(player) {
      return player.fireDirection === 'down'
    })
    var laneNumber = Math.floor(Math.random() * board.lanes.length)

    var timeNow = new Date().getTime()
    if (timeNow - this.bombDropTime >= this.bombDelay) {
      new Bullet(board, assets, board.lanes[laneNumber], npc)
      sounds.bullet.playSound()
      this.bombDropTime = timeNow
      var base = this.bombDelayBase
      var variance = this.bombDelayVariance
      this.bombDelay = base * (1 - variance) + (Math.random() * base * variance * 2)
